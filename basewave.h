...
typedef struct _SWEETLY_LOOPBACK_BUFFER
{
    PBYTE       Buffer;
    ULONG       Size;
    ULONG       WritePosition;
    ...
    KSPIN_LOCK  Lock;
    BOOLEAN     Initialized;
} SWEETLY_LOOPBACK_BUFFER, *PSWEETLY_LOOPBACK_BUFFER;
...
protected:
    ...
    static SWEETLY_LOOPBACK_BUFFER   s_LoopbackBuffer;
    static NTSTATUS                  InitializeLoopbackBuffer(_In_ ULONG PreferredSize);
    static void                      LoopbackWrite(_In_reads_bytes_(ByteCount) PBYTE Data, _In_ ULONG ByteCount);
    static void                      LoopbackRead(_Out_writes_bytes_(ByteCount) PBYTE Data, _In_ ULONG ByteCount);
