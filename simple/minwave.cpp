        ...
        else
        {
            if (PropertyRequest->Verb & KSPROPERTY_TYPE_SET)
            {
                    ...
                    (pKsFormat->DataFormat.Specifier == KSDATAFORMAT_SPECIFIER_WAVEFORMATEX))
                {
                    ...
                    if ((pWfx->nSamplesPerSec == MIN_SAMPLE_RATE) &&
                        (pWfx->nChannels == MAX_CHANNELS_PCM))
                    {
                        if (IsEqualGUIDAligned(pKsFormat->DataFormat.SubFormat, KSDATAFORMAT_SUBTYPE_PCM) &&
                            (pWfx->wFormatTag == WAVE_FORMAT_PCM) &&
                            (pWfx->wBitsPerSample == MIN_BITS_PER_SAMPLE_PCM) &&
                            (pWfx->cbSize == 0) &&
                        ...
                        else if (IsEqualGUIDAligned(pKsFormat->DataFormat.SubFormat, KSDATAFORMAT_SUBTYPE_IEEE_FLOAT) &&
